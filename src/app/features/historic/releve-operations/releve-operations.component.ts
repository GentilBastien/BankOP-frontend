...
export class ReleveOperationsComponent implements OnInit, AfterViewInit {
  ...
  public ngOnInit(): void {
    this.historicUsecases.refreshReleveOperations().pipe(
      switchMap(() => this.operationsChanges())
    ).subscribe();
    this.historicUsecases.filtersChanges().pipe(
      tap(filters => console.log()) //todo: CHANGER LA PASTILLE DE FILTRES ACTIFS
    ).subscribe()
  }
